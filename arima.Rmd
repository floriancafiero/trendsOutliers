...
```{r outlier detection}
library(tsoutliers)
allem_risk_gen.ts<- ts(allem_risk_gen,allemequency=1)
allem_risk_gen.ts.outliers <- tso(allem_risk.ts)
allem_risk_gen.ts.outliers
plot(allem_risk_gen.ts.outliers)
```
...
plot(allem_risk_spec.ts.outliers)
```
...
plot(aust_risk_gen.ts.outliers)
```
...
plot(aust_risk_spec.ts.outliers)
```
...
plot(calif_risk_gen.ts.outliers)
```
...
fr_risk_gen.ts.outliers <- tso(fr_risk.ts)
fr_risk_gen.ts.outliers
plot(fr_risk_gen.ts.outliers)
```
...
fr_risk_spec.ts.outliers <- tso(fr_risk.ts)
fr_risk_spec.ts.outliers
plot(fr_risk_spec.ts.outliers)
```
...
plot(ita_risk_spec.ts.outliers)
```
...
plot(serb_risk_gen.ts.outliers)
```
...
plot(serb_risk_spec.ts.outliers)
```
...
plot(wash_risk_gen.ts.outliers)
```
...
plot(wash_risk_spec.ts.outliers)
```
